...
def extract_course_info(file_path):
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            hours_tag = soup.find("p", string=lambda text: "Total Program Hours" in text if text else False)
            if hours_tag:
                course_data["Total Program Hours"] = hours_tag.text.split(":")[-1].strip()
            
            # Extract Textbooks (if any textbook information is available)
            textbooks_tag = soup.find("div", class_="textbooks")
            if textbooks_tag:
                course_data["Textbooks"] = textbooks_tag.text.strip()
            
            # Extract Prerequisites
            prerequisites_tag = soup.find("div", class_="prerequisites")
